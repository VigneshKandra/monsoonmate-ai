...
export function StepTransport({ transportation, onChange, errors }: StepTransportProps) {
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="flex items-center gap-2.5 mb-1.5">
        <Car className="size-5 text-cyan-400 shrink-0" />
        <h3 className="text-base sm:text-lg font-bold text-slate-100 tracking-tight">Transportation Mode</h3>
      </div>
      ...
      >
        {[
          { id: "car", label: "Own Car (4-Wheeler)" },
          { id: "bike", label: "Bike / Scooter (2-Wheeler)" },
          { id: "public", label: "Public Transport (Bus, Train, Metro)" },
          { id: "none", label: "No Personal/Public Vehicle (Walking only)" },
        ].map((option) => {
